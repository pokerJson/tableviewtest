...
//  XFileManager
//
//  Created by fantexix on 2017/5/8.
...
+ (NSArray *)listFilesInHomeDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self homePath] deep:deep];
}
+ (NSArray *)listFilesInDocumentDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self documentsPath] deep:deep];
}
+ (NSArray *)listFilesInLibraryDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self libraryPath] deep:deep];
}
+ (NSArray *)listFilesInPreferencesDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self preferencesPath] deep:deep];
}
+ (NSArray *)listFilesInCachesDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self cachesPath] deep:deep];
}
+ (NSArray *)listFilesInTmpDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self tmpPath] deep:deep];
}
...
+ (BOOL)clearCachesDirectory {
    ...
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[self cachesPath] stringByAppendingPathComponent:file];
...
+ (BOOL)clearTmpDirectory {
    ...
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[self tmpPath] stringByAppendingPathComponent:file];
        isSuccess &= [self removeItemAtPath:absolutePath];
    }
    return isSuccess;
}
