...
+ (UIImage *)grayscaleImageForImage:(UIImage*)image  rgb:(float)rgb {
    ...
    //通道  0:alpha  1:red 2:green 3:blue
    const int RED =1;
    const int GREEN =2;
    const int BLUE =3;
...
+ (UIImage *)transColorForImage:(UIImage*)image  rgb:(NSArray *)rgb {
    ...
    for(int y = 0; y < height; y++) {
        for(int x = 0; x < width; x++) {
            ...
            // set the pixels to gray
            if (rgbaPixel[0] != 0) {
                if (rgbaPixel[RED] != 255) {
                    rgbaPixel[RED] = [rgb[0] floatValue];
                }
                if (rgbaPixel[GREEN] != 255) {
                    rgbaPixel[GREEN] = [rgb[1] floatValue];
                }
                if (rgbaPixel[BLUE] != 255) {
                    rgbaPixel[BLUE] = 255;
                    rgbaPixel[BLUE] = [rgb[2] floatValue];
                }
            }
