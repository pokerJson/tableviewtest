...
@interface BListViewController ()<UITableViewDelegate,UITableViewDataSource,BListCellDelegate,UninterestedViewDelegate,ShareViewDelegate,MainTabBarControllerDelegate,MorePopViewDelegate>
...
@property(nonatomic, assign)BOOL preRefresh;//提前加载

@property(nonatomic, assign)float offs;

@property(nonatomic, assign)BOOL inital;
...
- (void)reachabilityMethod:(NSNotification*)notification {
    if (self.dataSource.count == 0 && [Reachability shared].reachable) {
        [self loadData];
    }
}
...
- (void)createTableView {
    ...
    _offs = _tableView.contentOffset.y;
    
    if (self.inital) {
        [self.tableView setContentOffset:CGPointMake(0, -self.scrollSwitch.offset)];
        self.inital = NO;
    }
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    _offs = scrollView.contentOffset.y;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == _tableView) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(viewController:offset:interval:)]) {
            [self.delegate viewController:self offset:offset interval:offset-_offs ];
        }
        _offs = offset;
        
        if (!_preRefresh) {
            if ((scrollView.contentSize.height < 3*scrollView.height && offset > scrollView.contentSize.height-1.5*scrollView.height)
                || (scrollView.contentSize.height > 3*scrollView.height && offset > scrollView.contentSize.height-2*scrollView.height)) {
                _preRefresh = YES;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ((BListCell *)cell).indexPath = indexPath;
}
...
- (void)morePopViewSelectedAtIndex:(int)index {
    ...
    switch (index) {
        case 0: {
            ...
            [[UserActionReport shared] uninterestedPost:_moreViewCell.model.ID ext:_moreViewCell.model.ext];
...
- (void)picMethod:(BListCell *)cell atIndex:(int)index {
    ...
    if (cell.model.type.intValue == 3) {
        ...
        [self.view.viewController.view addSubview:picView];
...
- (void)tabBarController:(MainTabBarController *)tabBarController refreshAtIndex:(NSInteger)index {
    if (index == 0) {
        ...
        [self.scrollSwitch showAddonsView];
    }
}
