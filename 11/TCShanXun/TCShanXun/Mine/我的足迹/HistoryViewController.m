...
@interface HistoryViewController ()<UITableViewDelegate,UITableViewDataSource,HistoryCellDelegate,UninterestedViewDelegate,ShareViewDelegate,MorePopViewDelegate>
...
@property(nonatomic, strong)HistoryCell * moreViewCell;
...
- (void)isHaveData {
    if (self.dataSource.count != 0) {
        ...
        if ([Reachability shared].reachable) {
            [_dataTipsView loadDataWithModel:@{@"label":@"暂无我的足迹"}];
        }else {
...
- (void)loadData {
    ...
    [HttpRequest get_RequestWithURL:MY_HISTORY_URL URLParam:param returnData:^(NSURLResponse *urlResponse, NSData *resultData, NSError *error) {
        if (!error) {
