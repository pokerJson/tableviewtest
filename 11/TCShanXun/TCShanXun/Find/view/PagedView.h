...
@protocol PagedViewDataSource;
@protocol PagedViewDelegate;


typedef enum{
    PagedViewOrientationHorizontal = 0,
    PagedViewOrientationVertical
}PagedViewOrientation;
 ...
 *  默认为横向
 */
@property (nonatomic,assign) PagedViewOrientation orientation;
...
@property (nonatomic,assign)   id <PagedViewDataSource> dataSource;
@property (nonatomic,assign)   id <PagedViewDelegate>   delegate;
...
@protocol  PagedViewDelegate<NSObject>
 ...
 *  @return <#return value description#>
 */
- (void)didSelectCell:(IndexBannerSubiew *)subView withSubViewIndex:(NSInteger)subIndex;

@end


@protocol PagedViewDataSource <NSObject>
