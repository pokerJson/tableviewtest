...
- (void)initialize{
    ...
    subviewClassName = @"IndexBannerSubiew";
...
- (void)queueReusableCell:(IndexBannerSubiew *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    IndexBannerSubiew *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case PagedViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                IndexBannerSubiew *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    IndexBannerSubiew *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, IndexBannerSubiew *cell) {
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[IndexBannerSubiew class]]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case PagedViewOrientationHorizontal://横向
...
- (IndexBannerSubiew *)dequeueReusableCell{
    IndexBannerSubiew *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(IndexBannerSubiew *)cell {
