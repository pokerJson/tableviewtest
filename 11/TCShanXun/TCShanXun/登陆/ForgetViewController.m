...
- (void)codeButtonMethod:(UIButton *)button {
    ...
    [HttpRequest get_RequestWithURL:FORGOT_SENDCODE_URL URLParam:dict returnData:^(NSURLResponse *urlResponse, NSData *resultData, NSError *error) {
        if (!error) {
...
- (void)submitButtonMethod:(UIButton *)button {
    ...
    [HttpRequest get_RequestWithURL:FORGOT_VERI_URL URLParam:param returnData:^(NSURLResponse *urlResponse, NSData *resultData, NSError *error) {
        if (!error) {
            id dic = [NSJSONSerialization JSONObjectWithData:resultData options:NSJSONReadingMutableContainers error:nil];
            NSLog(@"%@",dic);
