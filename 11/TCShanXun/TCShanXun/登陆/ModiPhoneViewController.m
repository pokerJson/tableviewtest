...
@interface ModiPhoneViewController ()<UITextFieldDelegate>
...
@implementation ModiPhoneViewController
...
- (void)submitButtonMethod:(UIButton *)button {
    ...
    [HttpRequest get_RequestWithURL:CHARGE_PHONE_3 URLParam:param returnData:^(NSURLResponse *urlResponse, NSData *resultData, NSError *error) {
        if (!error) {
            id dic = [NSJSONSerialization JSONObjectWithData:resultData options:NSJSONReadingMutableContainers error:nil];
            NSLog(@"%@",dic);
